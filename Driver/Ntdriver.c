	...
	case TC_IOCTL_OPEN_TEST:
		{
			...
			if (NT_SUCCESS (ntStatus))
			{
				...
				if (opentest->bDetectTCBootLoader || opentest->DetectFilesystem)
				{
					...
					{
						// Determine if the first sector contains a portion of the YourProduct Boot Loader
						...
						if (NT_SUCCESS (ntStatus))
						{
							...
							{
								// Search for the string "YourProduct"
...
BOOL IsVolumeClassFilterRegistered ()
{
	...
	if (NT_SUCCESS (status))
	{
		if (data->Type == REG_MULTI_SZ && data->DataLength >= 9 * sizeof (wchar_t))
		{
			...
			{
				if (memcmp (data->Data + i, L"yourproduct", 9 * sizeof (wchar_t)) == 0)
				{
...
NTSTATUS ReadRegistryConfigFlags (BOOL driverEntry)
{
	...
	RtlInitUnicodeString (&name, L"\\REGISTRY\\MACHINE\\SYSTEM\\CurrentControlSet\\Services\\yourproduct");
