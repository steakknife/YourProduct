	       ...
	       BOOL bRawDevice)
{
	...
	else
	{
		...
		// accidentally quick-formats a dismounted partition-hosted YourProduct volume as NTFS).
		...
		volumeType++)	
	{
		...
		if (ntStatus == STATUS_END_OF_FILE || IoStatusBlock.Information < TC_VOLUME_HEADER_EFFECTIVE_SIZE)
		{
			...
			// partition-hosted YourProduct volume and then tries to mount the volume using the embedded backup header.
