...
typedef struct
{
	...
	BOOL bDetectTCBootLoader;			// Whether the driver is to determine if the first sector contains a portion of the YourProduct Boot Loader
...
typedef struct
{
	...
	// is read-only (or mounted an outer/normal YourProduct volume as read only)
...
#define TC_UNIQUE_ID_PREFIX "YourProductVolume"
#define TC_MOUNT_PREFIX L"\\Device\\YourProductVolume"

#define NT_MOUNT_PREFIX DRIVER_STR("\\Device\\YourProductVolume")
#define NT_ROOT_PREFIX DRIVER_STR("\\Device\\YourProduct")
#define DOS_MOUNT_PREFIX DRIVER_STR("\\DosDevices\\")
#define DOS_ROOT_PREFIX DRIVER_STR("\\DosDevices\\YourProduct")
#define WIN32_ROOT_PREFIX DRIVER_STR("\\\\.\\YourProduct")

#define TC_DRIVER_CONFIG_REG_VALUE_NAME DRIVER_STR("YourProductConfig")
#define TC_ENCRYPTION_FREE_CPU_COUNT_REG_VALUE_NAME DRIVER_STR("YourProductEncryptionFreeCpuCount")
