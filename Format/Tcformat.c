...
// decrypt the system partition/drive and to uninstall YourProduct). The function removes the system encryption key 
// data ("volume header"), the YourProduct boot loader, restores the original system loader (if available),
	...
	case WM_INITDIALOG:
		{
			...
			strcat (szRescueDiskISO, "\\YourProduct Rescue Disk.iso");
...
retryCDDriveCheck:
			...
			else if (nCurPageNo == SYSENC_PRETEST_INFO_PAGE)
			{
				...
				/* Add the main YourProduct app to the system startup sequence (the YourProduct Background Task), which
				we need e.g. for notifications about prevented hibernation, about hidden OS leak protection, about 
				inconsistent hidden OS installs (YourProduct upgraded in the decoy system but not in the hidden one), etc.
...
// the YourProduct volume format version and the type of volume.
int AnalyzeHiddenVolumeHost (HWND hwndDlg, int *driveNo, __int64 hiddenVolHostSize, int *realClusterSize, __int64 *pnbrFreeClusters)
{
	...
	{
		// We do not support creating hidden volumes within volumes created by YourProduct 5.1a or earlier.
