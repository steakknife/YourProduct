...
namespace YourProduct
{
	...
	void UserInterface::CheckRequirementsForMountingVolume () const
	{
		...
		if (!Preferences.NonInteractive)
		{
			if (!SystemInfo::IsVersionAtLeast (2, 6, 24))
				ShowWarning (_("Your system uses an old version of the Linux kernel.\n\nDue to a bug in the Linux kernel, your system may stop responding when writing data to a YourProduct volume. This problem can be solved by upgrading the kernel to version 2.6.24 or later."));
		}
#endif // TC_LINUX
	}
...
#ifdef TC_LINUX
			if (string (volume.VirtualDevice).find ("/dev/mapper/yourproduct") != 0)
			{
#endif
	...
	wxString UserInterface::ExceptionToMessage (const exception &ex) const
	{
		...
		size_t pos = typeName.find ("YourProduct::");
		if (pos != string::npos)
		{
			return StringConverter::ToWide (typeName.substr (pos + string ("YourProduct::").size()))
...
#ifdef TC_MACOSX
		EX2MSG (HigherFuseVersionRequired,			_("YourProduct requires MacFUSE 1.3 or later."));
#endif
		...
		case CommandId::Help:
			{
				wstring helpText = StringConverter::ToWide (
					"Synopsis:\n"
					"\n"
					"yourproduct [OPTIONS] COMMAND\n"
					"yourproduct [OPTIONS] VOLUME_PATH [MOUNT_DIRECTORY]\n"
					"\n"
					"\n"
					"Commands:\n"
					"\n"
					...
					" 1) Path to the encrypted YourProduct volume.\n"
					...
					" command with option -o when a filesystem on a YourProduct volume is mounted.\n"
					...
					" Specifies comma-separated mount options for a YourProduct volume:\n"
					...
					"IMPORTANT:\n"
					"\n"
					"If you want to use YourProduct, you must follow the security requirements and\n"
					"security precautions listed in chapter 'Security Requirements and Precautions'\n"
					"in the YourProduct documentation (file 'YourProduct User Guide.pdf').\n"
					"\n"
					"\nExamples:\n\n"
					"Create a new volume:\n"
					"yourproduct -t -c\n"
					"\n"
					"Mount a volume:\n"
					"yourproduct volume.tc /media/yourproduct1\n"
					"\n"
					"Mount a volume as read-only, using keyfiles:\n"
					"yourproduct -m ro -k keyfile1,keyfile2 volume.tc\n"
					"\n"
					"Mount a volume without mounting its filesystem:\n"
					"yourproduct --filesystem=none volume.tc\n"
					"\n"
					"Mount a volume prompting only for its password:\n"
					"yourproduct -t -k \"\" --protect-hidden=no volume.tc /media/yourproduct1\n"
					"\n"
					"Dismount a volume:\n"
					"yourproduct -d volume.tc\n"
					"\n"
				...
				{
					wxDialog dialog (nullptr, wxID_ANY, _("YourProduct Command Line Help"), wxDefaultPosition);
